...
class HealthData(BaseModel):
    ...
    detailed: bool = False  # по умолчанию краткий план
...
async def get_plan(data_in: HealthData):
    ...
    # КРАТКИЙ ТЕЗИСНЫЙ ПЛАН (по умолчанию)
...
Составь КРАТКИЙ ТЕЗИСНЫЙ план тренировки. Без подробностей. Только самое важное.

Оценка состояния:
(1 предложение о готовности к тренировке)

Тренировка на сегодня:
(5-7 тезисов: какие упражнения или группы мышц, без весов и времени)

Важно сегодня:
(3-4 коротких тезиса по питанию, воде и восстановлению)
        ...
        async with httpx.AsyncClient(timeout=60) as http:
            resp = await http.post(
                ...
                json={
                    ...
                    "max_tokens": 2000 if data_in.detailed else 800
                }
            )
            result = resp.json()
